...
namespace SouthJLAInformationSystemC
{
    public partial class LoginForm : Form
    {
        ...
        private void SignInBtn_Click(object sender, EventArgs e)
        {
            ...
            SqlConnection con = new SqlConnection(stringFile.text);
            Console.WriteLine(stringFile.text);
            ...
            {
                SqlDataAdapter sda1 = new SqlDataAdapter("SELECT userClass, firstname, lastName FROM dbo.adminJLA WHERE username = '" + UsernameBox.Text + "'", con);
                /* in above line the program is selecting the whole data from table and the matching it with the user name and password provided by user. */
                DataTable dt1 = new DataTable(); //this is creating a virtual table  
                sda1.Fill(dt1);
                MainMenu mainMenu = new MainMenu(dt1.Rows[0][0].ToString(), dt1.Rows[0][1].ToString(), dt1.Rows[0][2].ToString());
                Hide();
                mainMenu.Show();
            }
            else
